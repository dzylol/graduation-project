...
RUN uv pip install --system --no-cache -r requirements.txt || \
    uv pip install --system --no-cache -i https://pypi.tuna.tsinghua.edu.cn/simple -r requirements.txt || \
    pip install --no-cache-dir -r requirements.txt
RUN MAX_JOBS=0 uv pip install --system --no-cache "causal-conv1d>=1.4.0" "mamba-ssm>=1.2.0" || \
    MAX_JOBS=0 uv pip install --system --no-cache -i https://pypi.tuna.tsinghua.edu.cn/simple "causal-conv1d>=1.4.0" "mamba-ssm>=1.2.0"
